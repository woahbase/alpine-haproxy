...
DPAVERSION ?= $(call get_gh_version,haproxytech/dataplaneapi,DPAVERSION)
...
BUILDFLAGS ?= \
	...
	--build-arg DPASRCARCH=$(call get_src_arch) \
	--build-arg DPAVERSION=$(if $(DPAVERSION),$(DPAVERSION),$(error DPAVERSION is not defined)) \
...
# runtime flags
MOUNTFLAGS := \
	# -v $(CURDIR)/config:/etc/haproxy \
	# -v $(CURDIR)/data:/var/lib/haproxy \
...
get_config: ## get default config from github
	curl -jSLN \
		-o $(CURDIR)/root/defaults/haproxy.cfg \
		https://raw.githubusercontent.com/haproxytech/haproxy-docker-alpine/refs/heads/main/3.0/haproxy.cfg \
	; \
	curl -jSLN \
		-o $(CURDIR)/root/defaults/dataplaneapi.yaml \
		https://raw.githubusercontent.com/haproxytech/dataplaneapi/refs/heads/master/configuration/examples/example-dataplaneapi.yaml \
	; \
	#
...
$(shell docker run --rm --pull=never \
	...
	--version \
	2>&1 \
	| grep '^HAProxy version' | awk -F'[ -]' '{print $$3}')
endef
...
SRC_ARCH_MAP := \
	...
	'armhf'  ) echo -n 'linux_arm'    ;; \
	'armv7l' ) echo -n 'linux_arm'    ;; \
	'i386'   ) echo -n 'linux_i386'   ;; \
	'loong64') echo -n 'N/A'          ;; \
	'ppc64le') echo -n 'linux_ppc64le';; \
	'riscv64') echo -n 'N/A'          ;; \
	's390x'  ) echo -n 'linux_s390x'  ;; \
	'x86_64' ) echo -n 'linux_x86_64' ;; \
    #
define get_src_arch
